...
class SmithUnit:
    ...
    def generate_and_test_commands(self):
        ...
        for i in range(self.num_seq):
            ...
            # 1. 随机选择一个func_函数执行
            func_random = random.randint(0, 485)
            func_rans.append(func_random)
            (func_id, func_pare) = find_operation_and_parameter(func_random)
        ...
        print(f"DEBUG: random is {func_rans}")
        print(f"DEBUG: actions is {func_names}")
...
if __name__ == "__main__":
    ...
    while not stop:
        ...
        with open(f"{options.directory}cov_turn.txt", "a") as file:
            file.write(f"turn is {i}, now time is {now - start}, cover line is {cov_line_turn}\n")
        i += 1

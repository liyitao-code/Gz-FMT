...
class OperatorSequenceManager:
    ...
    def sequence_to_vector(self, sequence, type=False):
        ...
        #         vector[index] += 1
        vector = np.zeros(sum(operator_parameter_counts))
        for a in sequence:
            vector[a] += 1
            # print(f"a is {a}")
        return vector
...
def prepare_cumulative_counts(operator_parameter_counts):
    cumulative_counts = []
    cumulative_sum = 0
    for count in operator_parameter_counts:
        cumulative_counts.append(cumulative_sum)
        cumulative_sum += count
    return cumulative_counts

# 算子参数前缀和
cumulative_counts = prepare_cumulative_counts(operator_parameter_counts)
...
def find_operation_and_parameter(x, cumulative_counts=cumulative_counts, operator_parameter_counts=operator_parameter_counts):
    # 正常遍历查找
    for operation_index, cumulative_count in enumerate(cumulative_counts):
        if cumulative_count <= x < cumulative_count + operator_parameter_counts[operation_index]:
            parameter_index = x - cumulative_count
            return (operation_index, parameter_index)
    
    # 如果x超出范围，抛出异常
    raise ValueError(f"x={x} is out of bounds for the given operator_parameter_counts.")
...
# 生成整个算子操作序列
def generate_action_sequence(state, actor, num_actions):
    ...
    for _ in range(num_actions):
        ...
        action_sequence.append(operator)
        action_log_probs.append(operator_log_prob)

    print(f"DEBUG: action sequence is {action_sequence}")
    print(f"DEBUG: action log_probs is {action_log_probs}")
...
class SmithUnit:
    ...
    def generate_and_test_commands_train(self, state, sequence_manager, actor, critic, actor_optimizer, critic_optimizer):
        state_dim = 5  # 状态共有5维
        ...
        action_sequence, action_log_probs = generate_action_sequence(state, actor, unit.num_seq)  # 修改：使用独立的 actor 模型
        action_para_sequence = []
        for action_all in action_sequence:
            action_para_sequence.append(find_operation_and_parameter(action_all))
        print("DEBUG: action sequence is " + str(action_para_sequence))
        ...
        i = 0
        for (operator, parameter) in action_para_sequence:
        ...
        actor_loss = -sum(action_log_probs) * advantage_tensor
        ...
        with open(f"{self.directory}/action_para.txt", 'a', encoding='utf-8') as file:
            file.write(str(action_para_sequence) + '\n')
...
if __name__ == "__main__":
    ...
    while not stop:
        ...
        reward, cov_line = unit.generate_and_test_commands_train(state, sequence_manager, actor, critic, actor_optimizer, critic_optimizer)

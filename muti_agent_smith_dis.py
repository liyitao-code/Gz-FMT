...
# 定义每个算子的参数数量
action_param_counts = {
    0: 1,    # RANDOM_LOAD_MODEL_XML
    1: 117,  # RANDOM_ADD_PLUGIN_XML
    2: 123   # RANDOM_ADD_MODEL_WITH_PLUGIN_XML
}

# 将带有参数的算子编号转为对应的网络的编号
action_agent_id = [-1, 0, 1]

# list形式定义每个算子的参数数量
operator_parameter_counts = [1, 117, 123]
...
class SimulatorAction:
    '''动作空间'''
    RANDOM_LOAD_MODEL_XML = 0
    RANDOM_ADD_PLUGIN_XML = 1
    RANDOM_ADD_MODEL_WITH_PLUGIN_XML = 2

    @staticmethod
    def perform_action(action):
        if action == SimulatorAction.RANDOM_LOAD_MODEL_XML:

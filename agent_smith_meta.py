...
META_STATE_DIM = 8 + len(METAMORPHIC_TEST_TYPES)


class MetaSimulatorState:
    def __init__(self, sdf_file_path, relation_counts=None):
        self.sdf_file_path = sdf_file_path
        self.relation_counts = list(relation_counts or [0] * len(METAMORPHIC_TEST_TYPES))
        self.model_count = 0
        self.plugin_count = 0
        self.world_plugin_count = 0
        self.joint_count = 0
        self.link_count = 0
        self.static_model_count = 0
        self.has_world = 0
        self.is_valid = 0
        self.extract_sdf_state()
    ...
    def extract_sdf_state(self):
        ...
        try:
            ...
            return
        self.is_valid = 1
        self.has_world = 1 if root.find(".//world") is not None else 0
        models = root.findall(".//model")
        self.model_count = len(models)
        self.plugin_count = len(root.findall(".//model/plugin"))
        self.world_plugin_count = len(root.findall(".//world/plugin"))
        self.joint_count = len(root.findall(".//joint"))
        self.link_count = len(root.findall(".//link"))
        self.static_model_count = sum(
            1 for model in models if (model.findtext("static") or "").strip().lower() == "true"
        )

    def to_vector(self):
        total_choices = sum(self.relation_counts)
        if total_choices:
            relation_rates = [count / total_choices for count in self.relation_counts]
        else:
            relation_rates = [0.0] * len(METAMORPHIC_TEST_TYPES)
        return [
            float(self.model_count),
            float(self.plugin_count),
            float(self.world_plugin_count),
            float(self.joint_count),
            float(self.link_count),
            float(self.static_model_count),
            float(self.has_world),
            float(self.is_valid),
            *relation_rates,
        ]
...
class MetaRelationSequenceManager:
    def __init__(self, action_count, history_size=100):
        self.action_count = action_count
        self.history = deque(maxlen=history_size)
        self.counts = [0] * action_count

    def add(self, action_index):
        self.history.append(action_index)
        self.counts[action_index] += 1

    def diversity_reward(self, action_index):
        if not self.history:
            return 0.0
        current_rate = self.counts[action_index] / len(self.history)
        return max(0.0, 1.0 - current_rate)
...
def select_meta_action(policy, epsilon=0.1):
    action_prob = policy.detach().cpu().numpy().flatten()
    ...
    if random.random() < epsilon:
        action = random.randrange(len(action_prob))
    else:
        action = int(np.random.choice(len(action_prob), p=action_prob))
    log_prob = torch.log(policy.squeeze(0)[action].clamp(min=1e-10))
    return action, log_prob


def calculate_meta_reward(result, diversity_reward=0.0):
    if not result or not result.get("executed", False):
        return -0.5 + diversity_reward
    reward = 0.1 + diversity_reward
    if result.get("crash_detected", False):
        reward += 4.0
    if result.get("test_passed") is False:
        reward += 4.0
    if result.get("playback_passed") is False:
        reward += 1.5
    return reward


def train_meta_policy(state, action_log_prob, reward, actor, critic, actor_optimizer, critic_optimizer):
    ...
    actor_loss = -action_log_prob * advantage.detach().squeeze()
...
def append_meta_training_metrics(path, row):
...
class SmithUnit:
    ...
    # 生成和测试命令（修改为执行蜕变测试）
    def generate_and_test_commands(self, test_type=None):
        # 初始化实验日志
        self.experiment_log = []
        experiment_start_time = time.time()

        def build_result(executed=False, test_passed=False, playback_passed=None, crash_detected=False):
            return {
                "test_type": test_type,
                "executed": executed,
                "test_passed": test_passed,
                "playback_passed": playback_passed,
        ...
        try:
            ...
            print("DEBUG: subprocess launch gz error")
            return build_result(executed=False)
        ...
        if test_type is None:
            test_type = random.choice(METAMORPHIC_TEST_TYPES)
        elif test_type not in METAMORPHIC_TEST_TYPES:
            raise ValueError(f"Unknown metamorphic test type: {test_type}")
        ...
        if process is None:
            return build_result(
                executed=test_result is not None,
                test_passed=test_passed,
                playback_passed=playback_success,
                crash_detected=crash_detected,
            )
        else:
            try:
...
if __name__ == "__main__":
    ...
    parser.add_option("--epsilon", dest="epsilon", type="float", default=0.1, help="epsilon for RL exploration")
    parser.add_option("--lr", dest="lr", type="float", default=0.001, help="learning rate for meta RL")
    parser.add_option("--meta-history-size", dest="meta_history_size", type="int", default=100, help="history size for relation diversity reward")
    parser.add_option("--random-meta", dest="random_meta", action="store_true", default=False, help="use random metamorphic relation selection")
    ...
    crashes = set()
    i = 0
    meta_relation_manager = MetaRelationSequenceManager(
        len(METAMORPHIC_TEST_TYPES),
        history_size=options.meta_history_size,
    )
    meta_actor = MetaActor(META_STATE_DIM, len(METAMORPHIC_TEST_TYPES))
    meta_critic = MetaCritic(META_STATE_DIM)
    meta_actor_optimizer = optim.Adam(meta_actor.parameters(), lr=options.lr)
    meta_critic_optimizer = optim.Adam(meta_critic.parameters(), lr=options.lr)
    meta_metrics_path = f"{options.directory}_meta_training_metrics.csv"
    ...
    # cov_line_turn = 0
    turn = -1
    while not stop and i < options.iteration:
        ...
        print("id = " + str(i + 1))
        meta_state = MetaSimulatorState(
            os.path.join(exp_dir, "a.sdf"),
            relation_counts=meta_relation_manager.counts,
        )
        if options.random_meta:
            action_index = random.randrange(len(METAMORPHIC_TEST_TYPES))
            action_log_prob = None
        else:
            state_tensor = torch.FloatTensor(meta_state.to_vector()).unsqueeze(0)
            policy = meta_actor(state_tensor)
            action_index, action_log_prob = select_meta_action(policy, epsilon=options.epsilon)
        test_type = METAMORPHIC_TEST_TYPES[action_index]
        diversity_reward = meta_relation_manager.diversity_reward(action_index)
        print(f"DEBUG: meta selected test_type={test_type}, diversity_reward={diversity_reward}")
        # Coverage tracking disabled
        # cov_line = unit.generate_and_test_commands()
        result = unit.generate_and_test_commands(test_type=test_type)
        reward = calculate_meta_reward(result, diversity_reward)
        if options.random_meta:
            actor_loss = 0.0
            critic_loss = 0.0
        else:
            actor_loss, critic_loss = train_meta_policy(
                meta_state,
                action_log_prob,
                reward,
                meta_actor,
        ...
        meta_relation_manager.add(action_index)
        append_meta_training_metrics(meta_metrics_path, {
            "iteration": i,
            "test_type": test_type,
            "reward": reward,
            "actor_loss": actor_loss,
            "critic_loss": critic_loss,
            "executed": result.get("executed") if result else False,
            "test_passed": result.get("test_passed") if result else False,
            "playback_passed": result.get("playback_passed") if result else None,
            "crash_detected": result.get("crash_detected") if result else False,
            "epsilon": options.epsilon,
        })
        print(f"DEBUG: meta reward={reward}, actor_loss={actor_loss}, critic_loss={critic_loss}")
